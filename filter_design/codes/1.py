...
# Given parameters
epsilons = [0.31, 0.45, 0.55, 0.6197]
...
plt.fill(passband_x, passband_y, 'b', alpha=0.3, edgecolor='none')

# Stopband
stopband_x = [1.4682, 2, 2, 1.4682]
stopband_y = [0, 0, stopband_upper_limit, stopband_upper_limit]
plt.fill(stopband_x, stopband_y, 'y', alpha=0.3, edgecolor='none')

plt.xlabel('$\Omega$_L')
plt.ylabel('|Ha LP($\Omega$)|')
plt.legend(loc='best', title='Passband and Stopband', labels=['Epsilon = 0.31', 'Epsilon = 0.45', 'Epsilon = 0.55', 'Epsilon = 0.6197', 'Passband', 'Stopband'])
plt.grid(True)
plt.savefig("1.png")
