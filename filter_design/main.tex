...
$\Omega_{p1} = 0.4899$, $ \Omega_{p2} = 0.3959$ and $\Omega_{s1} = 0.5177$, $\Omega_{s2} = 0.0.3764$
respectively.
...
\end{equation}
where $\Omega_0 = \sqrt{\Omega_{p1}\Omega_{p2}} = 0.4404
$ and $B = \Omega_{p2} - \Omega_{p1} = 0.094$.

Substituting $\Omega_{s1}$ and $\Omega_{s2}$ in \eqref{eq:freq_transform} we obtain the stopband edges of lowpass filter 
\begin{align}
    \Omega_{Ls1} &= \frac{\Omega_{s1}^2 - \Omega_0^2}{B\Omega_{s1}} = 1.5219\\
    \Omega_{Ls2} &= \frac{\Omega_{s2}^2 - \Omega_0^2}{B\Omega_{s2}} = -1.4775
\end{align}
...
\input{tables/filter}
we get $N\geq 4$ and $0.3043 \leq \epsilon \leq 0.6197$\\
\newpage
The below code plots \eqref{eq:mag_freq_response} for different values of $\epsilon$ .
\begin{lstlisting}
https://github.com/Pranav11003/EE1205/blob/main/filter_design/codes/1.py
\end{lstlisting}
\begin{figure}[H]
\centering
\includegraphics[width=1\columnwidth]{figs/1.png}
\caption{The Analog Low-Pass Frequency Response for $0.30435  \leq \epsilon \leq 0.6197$}
...
\begin{lstlisting}
https://github.com/Pranav11003/EE1205/blob/main/filter_design/codes/poles.c
\end{lstlisting}
...
\begin{lstlisting}
https://github.com/Pranav11003/EE1205/blob/main/filter_design/codes/2.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\end{align}
Substituting respective values in \eqref{eq:Gain_eq_LP} we get $G_{LP}=0.4052$
\begin{align}
     H_{a,LP}(s_L) &= \frac{0.4052}{\brak{s_L-s_1}\brak{s_L-s_2}\brak{s_L-s_3}\brak{s_L-s_4}}\\
     &= \frac{0.4052}{s_{L}^4 + 1.2794s_L^3 +  1.8257s_L^2 + 1.1403s_L +0.4250}\label{eq:design}
\end{align}
\begin{lstlisting}
https://github.com/Pranav11003/EE1205/blob/main/filter_design/codes/3.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\begin{align}
    H_{a,BP}\brak{s} &= \frac{3.1784 \times 10^{-5}s^4}{s^8 + 0.1203s^7 + 0.7919s^6 + 0.0709s^5 + 0.2320s^4 + 0.0138s^3 + 0.0298s^2 + 0.0009s + 0.0042} \label{eq:magnitude_bandpass_sdom}
\end{align}
...
\begin{lstlisting}
https://github.com/Pranav11003/EE1205/blob/main/filter_design/codes/4.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\begin{align}
    H_{d,BP}(z) &= \frac{G(z^8 - 4z^6 + 6z^4 - 4z^2 + 1)}{2.2638z^8 - 11.8454z^7 + 32.0038z^6 - 54.0454z^5 + 63.469z^4 - 51.3874z^3 + 28.949z^2 - 10.1842z + 1.852}
\end{align}

where $G=3.1784 \times 10^{-5}$\\  
\begin{lstlisting}
https://github.com/Pranav11003/EE1205/blob/main/filter_design/codes/5.py
\end{lstlisting}  
\begin{figure}[H]
\centering
...
\end{align}
\begin{lstlisting}
https://github.com/Pranav11003/EE1205/blob/main/filter_design/codes/6.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\end{enumerate}
\begin{lstlisting}
https://github.com/Pranav11003/EE1205/blob/main/filter_design/codes/7.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\begin{align}
    h_{BP}\brak{n} = 
\begin{cases} 
    \frac{\sin(w_{p1} n)}{n\pi} -\frac{\sin\brak{\omega_{p2}n}}{n\pi},  & \text{for } n\neq 0\\\
    \frac{\omega_{p1}-\omega_{p2}}{\pi} &\text{for} n= 0
\end{cases} \label{eq:h(n)desired_for_LPF}
\end{align}
\begin{align}
     \frac{\sin(\omega_{p1} n)}{n\pi} -\frac{\sin\brak{\omega_{p2}n}}{n\pi} &= 2\cos{\brak{\frac{\omega_{p1}n+\omega_{p2}n}{2}}}\sin{\brak{\frac{\omega_{p1}n-\omega_{p2}n}{2}}}\\
            &= \frac{2\cos{\brak{1.0407n\pi}}\sin{\brak{0.1141n\pi}}}{n\pi}
\end{align}
Multipying by window function we get :
\begin{align}
    h_{BP}\brak{n} = 
\begin{cases} 
   \frac{2\cos{\brak{1.0407n\pi}}\sin{\brak{0.1141n\pi}}}{n\pi},  & \text{for } -48\leq n \leq 48 \\
    0 &\text{otherwise}
\end{cases} \label{eq:h(n)desired_for_BPF}
\end{align}
\begin{lstlisting}
https://github.com/Pranav11003/EE1205/blob/main/filter_design/codes/8.py
\end{lstlisting}
\begin{figure}[H]
\centering
